...
__all__ = [ 'assert_isdone', # assert brep api is done
            ...
            # INTERPOLATION
            'points_to_bspline',
            'interpolate_points_to_spline',
            'interpolate_points_vectors_to_spline',
            # BUILD PATCHES
            'common_vertex',
            'midpoint',
            'center_boundingbox',
...
from ..Euclid import RP_Pnt_Array
from ..Euclid import RP_Pnt, gp_Vec, gp_Trsf
...
class assert_isdone(object):
    ...
    def __enter__(
        ...
        if self.to_check.IsDone():
            return self.to_check.Shape()
        else:
            raise Exception(self.error_statement)

    def __exit__(self, assertion_type, value, traceback): # for with
        return True
...
def center_boundingbox(shape):
    ...
    return midpoint(RP_Pnt(xmin, ymin, zmin), RP_Pnt(xmax, ymax, zmax))
...
def vertex2pnt(vertex):
    """returns a RP_Pnt from a TopoDS_Vertex"""
