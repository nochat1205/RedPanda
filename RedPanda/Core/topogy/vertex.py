...
from ..Euclid import RP_Pnt, RP_Vec, RP_Dir, RP_XYZ, RP_Pnt2d


class VertexAnalyst(TopoDS_Vertex, BaseObject):
    """
    wraps RP_Pnt
    """
    ...
    def __init__(self, vertex:TopoDS_Vertex):
        super(VertexAnalyst, self).__init__()
        ...
        self._pnt = BRep_Tool.Pnt(vertex)
        self._vertex = vertex
    ...
    @property
    def as_vec(self):
        """returns a RP_Vec version of self"""
        return RP_Vec(*self._pnt.Coord())

    @property
    def as_dir(self):
        """returns a RP_Dir version of self"""
        return RP_Dir(*self._pnt.Coord())

    @property
    def as_xyz(self):
        """returns a RP_XYZ version of self"""
        return RP_XYZ(*self._pnt.Coord())
    ...
    @property
    def as_2d(self):
        """returns a RP_Pnt2d version of self"""
        return RP_Pnt2d(*self._pnt.Coord()[:2])
