...
from OCC.Core.gp import gp_Dir, gp_Pnt, gp_Pnt2d, gp_Vec, gp_Ax2, gp_Pln, gp_Ax3
...
class Viewer2d(Display3d):
    ...
    def Create(
        ...
        # lights manager
        if create_default_lights: # default lights
            self.Viewer.SetDefaultLights()
            self.Viewer.SetLightOn()
        # objects manager Erase, Unhlight, Computed mode
        # Param: computer, MoveTo 是否更新 viewer.Update()

        # privileged plane management
        # grid 依赖于 privileged plane
