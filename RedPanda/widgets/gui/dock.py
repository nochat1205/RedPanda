...
class CollapsibleDockWidget(QDockWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 事件过滤器
        close.installEventFilter(self) # connect to eventFilter
        self.__closeButton = close

        self.__stack = AnimatedStackedWidget() # TODO:
    ...
    def setExpanded(self, IsExpanded):
        ...
        """
        if self.__expanded != IsExpanded:
            self.__expanded = IsExpanded
            if IsExpanded and self.__expandedWidget is not None:
                self.__stack.setCurrentWidget(self.__expandedWidget)
            elif not IsExpanded and self.__collapsedWidget is not None:
    ...
    def __fixMinimumWidth(self): # TODO:
        # type: () -> None
