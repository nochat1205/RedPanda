...
class qtViewer2d(qtBaseViewer):
    ...
    def InitDriver(self):
        ...
        aWindow = self.window()
        aWidth, aHeight = int(aWindow.width()), int(aWindow.height())
        self.SetUVGrid(-aWidth, aWidth, -aHeight, aHeight)
        # me 
    ...
    def GridEnable(self, u1=0, u2=0, v1=100, v2=100):
        ...
        aViewer.SetRectangularGridValues (-(u1+u2)/2, -(v1+v2)/2, (u2-u1)/10, (v2-v1)/10, 0)
    ...
    def SetUVGrid(self, u1, u2, v1, v2):
        ...
        length = int(min((u2-u1), (v2-v1)))
        self.TrihedronEnable(length/2)
